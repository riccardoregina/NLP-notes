\documentclass{csnotes_eng}

% ==============================================================================
% DOCUMENT SPECIFIC CUSTOMIZATIONS
% ==============================================================================

% --------------------------- START CUSTOMIZATIONS -----------------------------

% You can add any custom packages or commands specific to this document here

\usepackage{booktabs}

\setabstracttitle{Purpose of the Document}

\newcommand{\whitepage}{
  \newpage
  \thispagestyle{empty}
  \mbox{}
  \newpage
}

% ---------------------------- END CUSTOMIZATIONS ------------------------------

% ==============================================================================
% DOCUMENT INFORMATION
% ==============================================================================
\title{Statistical Learning - Natural Language Processing}
\author{Riccardo Regina}
\date{\today}

% Additional information for the title page
\university{University of Naples Federico II}
\department{Department of Electrical Engineering and Information Technology}
\course{Master's Degree in Computer Science}
\subject{Statistical Learning: Natural Language Processing}
\academicyear{Academic Year 2025/2026}

% ==============================================================================
% BIBLIOGRAPHY
% ==============================================================================
% Specify the .bib file with your bibliographic references
% Uncomment the following line when you have created the bibliography.bib file
\addbibresource{bibliography.bib}

% ==============================================================================
% INIZIO DOCUMENTO
% ==============================================================================

\begin{document}

\pagenumbering{gobble}

\maketitlepage{}
\whitepage{}

\input{_chapters/0_intro.tex}
\whitepage{}

\newgeometry{top=2cm}
\tableofcontents
\restoregeometry{}

\whitepage{}

\pagenumbering{arabic}
\setcounter{page}{1}

\input{_chapters/1_nlp}
\input{_chapters/2_features}
\input{_chapters/3_case_studies}
\input{_chapters/4_inputs}
\input{_chapters/5_language_modeling}
\input{_chapters/6_pretrained_word_representations}
\input{_chapters/7_rnn}
\input{_chapters/8_transformers}

\whitepage{}
\printbibliography
\makebackcover{}

\end{document}