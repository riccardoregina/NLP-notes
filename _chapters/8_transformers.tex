...
More precisely, the decoder is made up of \keyword{cross-attention} heads, instead of the already mentioned attention heads.
A cross-attention head is defined as
\[
...
\]
and may look equal to the standard Attention head, but the difference is that here \(\mathbf{Q}\) is taken from the encoder output \(\mathbf{Z}\) while \(\mathbf{K},\mathbf{Q}\) are taken from the current decoder generation.
