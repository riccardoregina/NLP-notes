...
In language processing, feature vectors $\mathbf{x}$ are derived from textual data, in order to reflect various linguistic properties of the text. This process of derivation is called \keyword{feature extraction} and is done by a feature function $f$.
...
\begin{definition}{}
    A \keyword{word} is the smallest unit of meaning, e.g., \textit{New York}, \textit{forgets}. In general, we distinguish between words and tokens. A \keyword{token} is the output of a tokenizer and can be a single word, multiple words, or part of a word. However, we will often use \textit{word} and \textit{token} as synonyms.
\end{definition}

\section{Features for NLP}
As words and letters are discrete items, features in NLP often take the form of indicators or counts. An \keyword{indicator feature} takes a value $0$ or $1$ depending on the existence of a condition (e.g., the word \textit{dog} is present). A \keyword{count} takes a value depending on the number of occurrences of some event (e.g., the word \textit{dog} appears 20 times in the document).
...
\begin{itemize}
    \item \textbf{Windows}: It is common to focus on the immediate context of a word by considering a \keyword{window} surrounding it, i.e., $k$ words on each side, where typical values for $k$ are $2, 5, 10$. A window can be seen as a smaller bag of words. Since it is smaller, we may also consider the positions of words within it, which can provide useful information. \textbf{We take the identity of words as features}, e.g., for \textit{the brown fox \textbf{jumped} over the lazy dog}, the set of features is $\{w_{-2}=\text{brown}, w_{-1}=\text{fox}, w_{+1}=\text{over}, w_{+2}=\text{the}\}$.
...
\subsection{Ngrams}
A special case of feature combination is \keyword{ngrams}, consecutive word sequences of length $n$. A bag-of-bigrams representation is more powerful than bag-of-words because it can capture bigrams like \textit{New York} or \textit{not good}.

A priori, it is hard to know the optimal value for $n$. The common solution is to consider all ngrams of a given length \(n\) and let the model's regularization discard the less interesting ones by assigning them very low weights.

\begin{note}{Regularization}
    Regularization is a technique used to penalize large weights in a model, thereby encouraging to assign zero weight to irrelevant features and incrementing the weight value to very important features only.
    This is done by adding a penalty term to the loss function proportional to the absolute value of a weight (\(L_1\) regularization) or its squared value (\(L_2\) regularization) or both (ElasticNet).
\end{note}
